...
pub async fn get_song_lyrics(music_user_token: &str, storefront: &str, song_id: &str) -> Result<SongLyrics, String> {
    ...
    call_with_dev_token_retry("get_song_lyrics", || {
        ...
        async move {
            ...
            // Extract timestamped text lines from TTML
...
///
/// TTML `<p>` tags carry `begin` attributes like `begin="00:01.234"`.
/// Returns `Vec<(Option<timestamp>, text)>`.
fn extract_lyrics_from_ttml(ttml: &str) -> Vec<(Option<String>, String)> {
    ...
    // Check if TTML has timestamps at all
...
///
/// Handles: "5.347" (raw seconds), "01:23.456" (MM:SS), "1:02:03.456" (HH:MM:SS).
