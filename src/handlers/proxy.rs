...
use super::{
    ...
    get_developer_token, invalidate_developer_token, is_allowed_apple_host, read_user_music_token,
    save_user_music_token,
};
...
pub async fn proxy_apple_music_api(
    ...
    // Retry once on 401 when no user token (dev token likely expired)
    let mut dev_token = get_developer_token().await?;
    for attempt in 0..2 {
        ...
        // Retry on 401 with no user token — dev token is likely expired
        if status == 401 && music_user_token.is_none() && attempt == 0 {
            info!("[AppleMusic] Proxy got 401 (no user token) — refreshing dev token and retrying");
            invalidate_developer_token().await;
            dev_token = get_developer_token().await?;
            continue;
        }
